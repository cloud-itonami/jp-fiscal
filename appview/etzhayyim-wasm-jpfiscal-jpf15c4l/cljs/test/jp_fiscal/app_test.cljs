(ns jp-fiscal.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [jp-fiscal.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held,
            corrected for wrangler.jsonc's actual routes/vars/xrpc"
    (rf/dispatch-sync [:initialize-db])
    (is (= app/default-db @rf-db/app-db))
    (is (= "Jpfiscal Jpf15c4l" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-wasm-jpfiscal-jpf15c4l" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-jp-fiscal" @(rf/subscribe [:app/project])))
    (is (= "appview" @(rf/subscribe [:app/kind])))
    (is (= 2 @(rf/subscribe [:app/route-count])))
    (is (= ["jpf15c4l.etzhayyim.com/*" "jp-fiscal.etzhayyim.com/*"]
           @(rf/subscribe [:app/routes])))
    (is (= ["APP_ACTOR_HANDLE" "APP_CAPABILITIES" "APP_DEPLOY_AT" "APP_DEPLOY_SHA"
    ...
    (is (= "appview/etzhayyim-wasm-jpfiscal-jpf15c4l/cljs/src/jp_fiscal/app.cljs"
